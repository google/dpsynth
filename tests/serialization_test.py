...
"""Unit tests for standalone serialization in serialization.py."""
...
class SerializationTest(parameterized.TestCase):
  ...
  def test_mst_config_roundtrip(self):
    ...
    yaml_str = serialization.to_yaml(config)
    loaded = serialization.from_yaml(yaml_str)
    self.assertEqual(loaded, config)
    self.assertEqual(
        serialization.from_yaml(yaml_str, expected_cls=mst.MSTConfig), config
    )
  ...
  def test_tabular_config_roundtrip(self):
  ...
  def test_initializer_configs_roundtrip(self):
    ...
    yaml_str = serialization.to_yaml(num_init)
    self.assertEqual(serialization.from_yaml(yaml_str), num_init)

    cat_init = initialization.CategoricalInitializerConfig(
        attribute=domain.CategoricalAttribute(possible_values=['A', 'B']),
    )
    yaml_str = serialization.to_yaml(cat_init)
    self.assertEqual(serialization.from_yaml(yaml_str), cat_init)

    open_init = initialization.OpenSetInitializerConfig(
        attribute=domain.OpenSetCategoricalAttribute(),
        min_count=5,
    )
    yaml_str = serialization.to_yaml(open_init)
    self.assertEqual(serialization.from_yaml(yaml_str), open_init)
  ...
  def test_schema_roundtrip(self):
    ...
    yaml_str = serialization.to_yaml(schema)
    loaded = serialization.from_yaml(yaml_str)
    self.assertEqual(loaded, schema)

  def test_attribute_dict_roundtrip(self):
    attr = domain.NumericalAttribute(min_value=10, max_value=50)
    d = serialization.to_dict(attr)
    self.assertEqual(d['type'], 'NumericalAttribute')
    self.assertEqual(serialization.from_dict(d), attr)
  ...
  def test_file_io_roundtrip(self):
    ...
    serialization.to_yaml_file(config, tmp_path)
    loaded = serialization.from_yaml_file(tmp_path)
    self.assertEqual(loaded, config)
  ...
  def test_missing_type_raises(self):
    with self.assertRaises(ValueError):
      serialization.from_yaml('pgm_iters: 1000\n')

  def test_unknown_type_raises(self):
    with self.assertRaises(ValueError):
      serialization.from_yaml('type: NonExistentConfig\n')
