...
def run_mechanism(
  ...
  max_rounds = config.max_rounds or 16 * len(data.domain)
  rho_per_round = zcdp_rho / max_rounds

  if initial_measurements is None:
    rho_remaining -= config.one_way_budget_fraction * zcdp_rho
  ...
  while not terminate:
    ...
    fraction = config.select_budget_fraction
    ...
    size_limit = config.max_model_size * (zcdp_rho - rho_remaining) / zcdp_rho
    ...
    if np.linalg.norm(new_estimate - old_estimate, ord=1) <= threshold:
      ...
      rho_per_round *= config.anneal_factor
      fraction = config.select_budget_fraction
      sigma = accounting.zcdp_gaussian_sigma((1 - fraction) * rho_per_round)
