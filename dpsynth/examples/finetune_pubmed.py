...
_BATCH_SIZE = flags.DEFINE_integer('batch_size', 4, 'Batch size for sampling.')
...
def train() -> dp_sft.FineTuneResult:
  ...
  data = datasets.load_dataset(_DATASET, split='train', trust_remote_code=True)
  abstracts = [t.strip() for t in data['abstract'] if t and t.strip()]
...
def sample(result: dp_sft.FineTuneResult) -> None:
  ...
  try:
    batched_sampler = sampler.gemma4_sampler
  except AttributeError:
    batched_sampler = sampler.sampler

  out_path = epath.Path(_WORKDIR.value) / 'synthetic_abstracts.jsonl'
  prompts = [_INSTRUCTION] * _NUM_SAMPLES.value
  with out_path.open('w') as f:
    for i in range(0, len(prompts), _BATCH_SIZE.value):
      batch = prompts[i : i + _BATCH_SIZE.value]
      batch_size = len(batch)
      # Pad undersized last batch to avoid recompilation.
      if batch_size < _BATCH_SIZE.value:
        padding = [batch[-1]] * (_BATCH_SIZE.value - batch_size)
        batch = batch + padding

      batch_responses = typing.cast(
          list[str],
          batched_sampler.sample(
              batch,
              sharding=kd.sharding.FIRST_DIM,
              rng=_SEED.value + i,
          ),
      )
      # Trim padded responses.
      batch_responses = batch_responses[:batch_size]

      for j, abstract in enumerate(batch_responses):
